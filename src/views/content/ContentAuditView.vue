  ...
  <div class="content-audit-view">
      ...
      >
        ...
        <template #op="{ row }">
          <div class="op-btns">
            ...
            <!-- 内容类型时显示跳转按钮 -->
            <span v-if="row.targetType === '0' || row.targetType === 0" class="op-btn default" @click="goToContentDetail(row.targetId)">
              <t-icon name="jump" /> 内容
            </span>
      ...
      <t-form :data="auditForm" :rules="auditRules" ref="auditFormRef" v-if="auditDialogVisible">
        <t-form-item label="审核意见" name="auditReason">
          <t-input
            ...
            :placeholder="auditAction === 'approve' ? '请输入审核通过意见（可选）' : '请输入拒绝原因'"
            :maxlength="500"
          />
        </t-form-item>
      </t-form>
    ...
    >
      <div class="detail-content" v-if="currentDetail">
        <!-- 审核信息 -->
        <t-descriptions title="审核信息" :column="1" bordered>
          <t-descriptions-item label="审核ID">{{ currentDetail.id }}</t-descriptions-item>
          <t-descriptions-item label="目标类型">
          ...
          <t-descriptions-item label="审核状态">
            <t-tag :theme="STATUS_MAP[currentDetail.auditStatus]?.theme" variant="light-outline">
              {{ STATUS_MAP[currentDetail.auditStatus]?.text }}
            </t-tag>
          </t-descriptions-item>
          <t-descriptions-item label="审核时间">{{ currentDetail.createTime }}</t-descriptions-item>
          <t-descriptions-item label="审核人ID">{{ currentDetail.auditBy || '-' }}</t-descriptions-item>
          <t-descriptions-item label="审核意见">
            <span :class="{ 'text-red-500': currentDetail.auditStatus === 2 }">
        ...
        <!-- 内容详情（内容审核时显示） -->
        <t-descriptions v-if="currentDetail.targetType === '0' || currentDetail.targetType === 0" title="内容详情" :column="1" bordered style="margin-top: 16px;">
          <t-descriptions-item label="内容ID">{{ currentDetail.targetId }}</t-descriptions-item>
          <t-descriptions-item label="标题">{{ currentDetail.contentTitle || '-' }}</t-descriptions-item>
          <t-descriptions-item label="内容类型">
          ...
          <t-descriptions-item label="描述">{{ currentDetail.contentDescription || '-' }}</t-descriptions-item>
          <t-descriptions-item label="封面">
            <img v-if="currentDetail.contentCoverUrl" :src="currentDetail.contentCoverUrl" style="max-width: 200px; border-radius: 4px;" />
            <span v-else>-</span>
          </t-descriptions-item>
          <t-descriptions-item label="作者ID">{{ currentDetail.contentAuthorId || '-' }}</t-descriptions-item>
        </t-descriptions>
        <t-button v-if="currentDetail.targetType === '0' || currentDetail.targetType === 0" theme="primary" variant="outline" style="margin-top: 16px;" @click="goToContentDetail(currentDetail.targetId)">
          <template #icon><t-icon name="jump" /></template>
          查看内容详情
        </t-button>

        <!-- 评论详情（评论审核时显示） -->
        <t-descriptions v-if="currentDetail.targetType === '1' || currentDetail.targetType === 1" title="评论详情" :column="1" bordered style="margin-top: 16px;">
          <t-descriptions-item label="评论ID">{{ currentDetail.targetId }}</t-descriptions-item>
          <t-descriptions-item label="评论内容">{{ currentDetail.commentContent || '-' }}</t-descriptions-item>
          <t-descriptions-item label="评论用户ID">{{ currentDetail.commentUserId || '-' }}</t-descriptions-item>
          <t-descriptions-item label="所属内容">{{ currentDetail.commentContentTitle || '-' }}</t-descriptions-item>
        </t-descriptions>
      </div>
    </t-drawer>
  </div>
